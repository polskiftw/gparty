...
if ((git -C $source rev-parse HEAD).Trim() -ne $commit) {
  throw "libhevc checkout did not resolve to the pinned commit"
}

# AOSP libhevc has no supported Windows target. Keep the adaptation deliberately
# narrow: replace only its platform/thread portability layer and remove Unix
# libm from the upstream sample/test link line. Codec implementation stays
# otherwise byte-for-byte at the pinned upstream commit.
Copy-Item apps/gdupe/experiments/libhevc/ithread_windows.c `
  (Join-Path $source "common/ithread.c") -Force
Copy-Item apps/gdupe/experiments/libhevc/ihevc_platform_macros_msvc.h `
  (Join-Path $source "common/x86/ihevc_platform_macros.h") -Force
...
if ((Get-Content $utilsPath -Raw) -match 'link_libraries\(Threads::Threads m\)') {
  throw "Failed to remove Unix libm linkage from libhevc"
}
...
if (-not (Test-Path -LiteralPath $library -PathType Leaf)) {
  throw "libhevc static decoder library was not produced"
}
if (@(Get-ChildItem $build -Recurse -File -Filter *.dll).Count -ne 0) {
  throw "libhevc unexpectedly produced a DLL"
}
...
if ($directives -notmatch '(?i)LIBCMT') { throw "libhevc is not using the static MSVC CRT" }
if ($directives -match '(?i)MSVCRT') { throw "libhevc requests the dynamic MSVC CRT" }
...
License: Apache-2.0
Build: static libhevcdec.lib, MSVC x64, /MT
Windows adaptation:
...
Write-Host "Built static libhevc SDK at $sdk"

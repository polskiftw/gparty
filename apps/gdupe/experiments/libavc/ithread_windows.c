/*
 * Windows thread abstraction shim for AOSP libavc.
 *
 * Implements libavc's ithread API with Win32 primitives so the decoder can be
 * statically built with MSVC without pthreads. Codec logic is untouched.
 */

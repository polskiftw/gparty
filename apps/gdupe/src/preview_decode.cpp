...
public:
  explicit FileMkvReader(const std::filesystem::path &path)
    ...
    if (!stream_)
      throw std::runtime_error("Could not open WebM file for preview");
  }
...
PreviewDecodeResult decode_webm_preview(
    const std::filesystem::path &path, std::stop_token stop,
    const PreviewFrameCallback &callback) {
  ...
  result.duration_ns = std::max<std::int64_t>(0, segment->GetDuration());
  NvdecPacketDecoder decoder(nvdec_codec_for_webm(codec), NvdecOutput::preview);
  feed_webm_codec_private(*video_track, codec, decoder);

  bool keep_going = true;
  const NvdecBgraFrameCallback on_nvdec_frame = [&](NvdecBgraFrame frame) {
    ...
    converted.timestamp_ns = std::max<std::int64_t>(0, frame.timestamp);
  ...
  while (entry && !entry->EOS() && keep_going && !stop.stop_requested()) {
    ...
    for (int frame_index = 0;
         frame_index < block->GetFrameCount() && keep_going &&
         !stop.stop_requested();
         ++frame_index) {
      ...
      decoder.decode_bgra(packet, timestamp_ns, on_nvdec_frame);
    }
  ...
  if (keep_going && !stop.stop_requested())
    decoder.flush_bgra(on_nvdec_frame);
  if (result.decoded_frames == 0 && !stop.stop_requested())
    throw std::runtime_error("WebM contained no NVDEC-decodable preview frames");
  return result;
}
...
public:
  explicit Mp4FileReader(const std::filesystem::path &path)
    ...
    if (!stream_)
      throw std::runtime_error("Could not open MP4/M4V file for preview");
...
PreviewDecodeResult decode_mp4_preview(
    const std::filesystem::path &path, std::stop_token stop,
    const PreviewFrameCallback &callback) {
  ...
  PreviewDecodeResult result;
  result.duration_ns = ticks_to_ns(track_duration_ticks(track), track.timescale);
  NvdecPacketDecoder decoder(codec.hevc ? NvdecCodec::hevc : NvdecCodec::h264,
                             NvdecOutput::preview);
  decoder.feed_header(codec.parameter_sets);

  std::vector<std::int64_t> timestamps(1, 0);
  timestamps.reserve(static_cast<std::size_t>(track.sample_count) + 1U);
  ...
  const NvdecBgraFrameCallback on_nvdec_frame = [&](NvdecBgraFrame frame) {
    ...
    std::int64_t timestamp_ns = last_timestamp;
    if (frame.timestamp > 0 &&
        static_cast<std::uint64_t>(frame.timestamp) < timestamps.size())
      timestamp_ns = timestamps[static_cast<std::size_t>(frame.timestamp)];
    timestamp_ns = std::max<std::int64_t>(0, timestamp_ns);
    last_timestamp = std::max(last_timestamp, timestamp_ns);
  ...
  for (unsigned sample_index = 0;
       sample_index < track.sample_count && keep_going &&
       !stop.stop_requested();
       ++sample_index) {
    ...
    const std::int64_t token = static_cast<std::int64_t>(timestamps.size() - 1U);
    decoder.decode_bgra(annexb, token, on_nvdec_frame);
  }

  if (keep_going && !stop.stop_requested())
    decoder.flush_bgra(on_nvdec_frame);
  if (result.decoded_frames == 0 && !stop.stop_requested())
    throw std::runtime_error("MP4/M4V contained no decodable preview frames");
  return result;
}
...
PreviewDecodeResult decode_video_preview_once(
  ...
  if (extension == "webm")
    return decode_webm_preview(path, stop, callback);
  if (extension == "mp4" || extension == "m4v")
    return decode_mp4_preview(path, stop, callback);
  throw std::runtime_error("Unsupported video preview extension: " + extension);
}

...
class GdupeStore {
public:
  explicit GdupeStore(const std::filesystem::path &path);
  ~GdupeStore();
  GdupeStore(const GdupeStore &) = delete;
  GdupeStore &operator=(const GdupeStore &) = delete;
